...
def analyse(config: AnalyseConfig) -> dict[str, Any]:
    ...
    if config.psd_mc_grid not in ("uniform", "octree"):
        raise ValueError("psd_mc_grid must be one of: uniform, octree")
    if config.psd_mc_grid == "octree" and config.no_octree:
        raise ValueError("psd_mc_grid=octree requires octree to be enabled")
        ...
        if not np.any(acc):
            ...
            if config.psd_method in ("mc", "both"):
                ...
                voxel_psd_data, promoted_fraction = get_psd_from_voxels_mc(
                    ...
                    mc_grid=config.psd_mc_grid,
                    grid_mask=grid_mask,
                    oct_soa_tuple=oct_soa_tuple,
                )
                logger.info(
